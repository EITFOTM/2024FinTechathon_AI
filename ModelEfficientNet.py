...
class SqueezeExcitation(nn.Module):
    ...
    def __init__(self,
        ...
        squeeze_c = input_c // squeeze_factor  # 输入该MBConv特征矩阵channels的1/4
...
class InvertedResidual(nn.Module):
    def __init__(self,
        ...
        # expand
        # 只有expand_ratio≠1才使用第一个1×1卷积层
        if cnf.expanded_channels != cnf.in_channels:
            layers.update({"expand_conv": ConvBNActivation(cnf.in_channels,
        ...
        # depthwise
        layers.update({"dwconv": ConvBNActivation(cnf.expanded_channels,
                                                  cnf.expanded_channels,
        ...
        if cnf.use_se:
            layers.update({"se": SqueezeExcitation(cnf.in_channels,
                                                   cnf.expanded_channels)})

        # project
        layers.update({"project_conv": ConvBNActivation(cnf.expanded_channels,
                                                        cnf.out_channels,
                                                        kernel_size=1,
                                                        norm_layer=norm_layer,
...
class EfficientNet(nn.Module):
    def __init__(self,
        ...
        # in_channel, out_channel, kernel_size, exp_ratio, strides, use_SE, drop_connect_rate, repeats
        default_cnf = [[32, 16, 3, 1, 1, True, drop_connect_rate, 1],
                       [16, 24, 3, 2, 6, True, drop_connect_rate, 2],
                       [24, 40, 5, 2, 6, True, drop_connect_rate, 2],
                       [40, 80, 3, 2, 6, True, drop_connect_rate, 3],
                       [80, 112, 5, 1, 6, True, drop_connect_rate, 3],
                       [112, 192, 5, 2, 6, True, drop_connect_rate, 4],
                       [192, 320, 3, 1, 6, True, drop_connect_rate, 1]]
        ...
        for stage, args in enumerate(default_cnf):
            ...
            for i in range(round_repeats(cnf.pop(-1))):
                if i > 0:
                    ...
                    cnf[3] = 1  # strides
                    cnf[0] = cnf[1]  # input_channel equal output_channel
        ...
        # first conv
        layers.update({"stem_conv": ConvBNActivation(in_channels=3,
        ...
        layers.update({"top": ConvBNActivation(in_channels=last_conv_input_c,
                                               out_channels=last_conv_output_c,
                                               kernel_size=1,
                                               norm_layer=norm_layer)})
